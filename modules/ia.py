...
class TrafficAI:
    ...
    def __init__(
        ...
        self.front_stop_dist = max(52.0, self.size * 0.42)
        self.front_slow_dist = max(110.0, self.size * 0.95)
    ...
    def _apply_player_like_turning(self, desired_angle: float, dt: float) -> None:
        # Match player-style steering feel: harder turning at high speed.
        speed_ratio = min(1.0, max(0.0, self.speed) / max(1.0, self.max_speed))
        steer_factor = 1.0 / (1.0 + 4.0 * speed_ratio)
        low_speed_limit = min(1.0, max(0.0, self.speed) / 200.0)
    ...
    def _enforce_road_constraint(self, game_map: Any, navigator: MapNavigator, dt: float) -> None:
        ...
        on_road = bool(game_map.is_road_at(cx, cy))
        ...
        target_x = road_cx - self.size * 0.5
        target_y = road_cy - self.size * 0.5

        snap_speed = _clamp(dt * 4.0, 0.0, 1.0)
        self.x += (target_x - self.x) * snap_speed
        self.y += (target_y - self.y) * snap_speed
        self.speed *= 0.65
    ...
    def update(
        ...
        if not forward_open:
            if right_open and left_open:
                ...
                turn_delta = -90.0
                do_turn = True
        elif self._junction_cooldown <= 0.0:
        ...
        # Right-side lane preference while preserving stable cardinal heading.
        lane_bias = 0.0
        if right_d <= self.right_min_dist:
            lane_bias -= self.lane_bias_max_deg
        else:
            right_err = right_d - self.right_target_dist
            lane_bias += _clamp(right_err / max(8.0, self.right_target_dist), -1.0, 1.0) * (
                self.lane_bias_max_deg * 0.70
            )
            if left_d + 8.0 < right_d:
                lane_bias += self.lane_bias_max_deg * 0.25

        obstacle_bias = _clamp(self._avoid_obstacles(obs) * 0.08, -6.0, 6.0)
        desired_angle = _normalize_angle_deg(target_heading + lane_bias + obstacle_bias)
...
class RobberAI(PursuitAI):
    ...
    def update(
        ...
        """Very light direct chase: always steer straight toward player target."""
        if dt <= 0.0:
            return
        ...
        target_pos = self._target_position()
        if target_pos is None:
            self._update_motion_towards(None, 0.0, dt, game_map, navigator, [])
            self.ai_state = "idle"
            return
        ...
        # Keep robbers dangerous: high baseline speed with short-range boost.
        desired_speed = min(self.max_speed, self.cruise_speed * 0.2)
        if dist <= 360.0:
            desired_speed = min(self.max_speed, desired_speed + 85.0)
        if dist <= self.ram_distance:
            desired_speed = min(self.max_speed, desired_speed + self.ram_speed_bonus * 0.6)

        self._update_motion_towards(target_pos, desired_speed, dt, game_map, navigator, [])
        self.ai_state = "chase"
...
class AIManager:
    ...
    def spawn_traffic(
        ...
        attempts = 0
        max_attempts = max(18, target_count * 24)
        ...
        while len(spawned) < target_count and attempts < max_attempts:
            ...
            existing = list(self.traffic_agents.values()) + spawned + list(self.pursuit_agents.values())
            if not self._is_agent_spawn_valid(ai, game_map, existing):
                continue
    ...
    def spawn_pursuit_enemy(
        ...
        target_count = max(0, count)
        attempts = 0
        max_attempts = max(16, target_count * 24)
        ...
        while len(spawned) < target_count and attempts < max_attempts:
            ...
            existing = list(self.traffic_agents.values()) + list(self.pursuit_agents.values()) + spawned
            if not self._is_agent_spawn_valid(ai, game_map, existing):
                continue
    ...
    def _is_agent_spawn_valid(self, agent: TrafficAI, game_map: Any, others: Optional[Iterable[TrafficAI]] = None) -> bool:
        ...
        for other in others or []:
            if _distance(
                (agent.x + agent.size * 0.5, agent.y + agent.size * 0.5),
                (other.x + other.size * 0.5, other.y + other.size * 0.5),
            ) < agent.size * 0.9:
                return False
    ...
    def _sample_road_spawn(
        ...
        for _ in range(max_tries):
            ...
            # Keep global density higher in map center.
            if self._rng.random() < self.traffic_center_bias:
                angle = self._rng.random() * math.tau
                radius = center_radius * math.sqrt(self._rng.random())
                wx = world_center[0] + math.cos(angle) * radius
                wy = world_center[1] + math.sin(angle) * radius
            elif focus and self._rng.random() < 0.8:
                fx, fy = self._rng.choice(focus)
                angle = self._rng.random() * math.tau
                radius = self._rng.uniform(self.traffic_spawn_min_distance, self.traffic_spawn_radius)
                wx = fx + math.cos(angle) * radius
                wy = fy + math.sin(angle) * radius
            else:
                tx = self._rng.randrange(0, w)
                ty = self._rng.randrange(0, h)
                if navigator.is_road_tile(tx, ty):
                    ...
                    if focus:
                        ...
                        if d2 < self.traffic_spawn_min_distance * self.traffic_spawn_min_distance:
                            continue
                        if d2 > self.traffic_spawn_radius * self.traffic_spawn_radius * 1.4:
                            continue
                    return spawn
                continue

            tile = navigator.world_to_tile(wx or 0.0, wy or 0.0)
            road_tile = navigator.find_nearest_road_tile(tile, max_radius=8)
            if road_tile is None:
                continue

            spawn = navigator.tile_to_world(road_tile[0], road_tile[1], centered=True)
            if focus:
                d2 = self._min_focus_distance_sq(spawn, focus)
                if d2 < self.traffic_spawn_min_distance * self.traffic_spawn_min_distance:
                    continue
    ...
    def update_all(
        ...
        neighbor_radius_sq = self.obstacle_neighbor_radius * self.obstacle_neighbor_radius
        ...
        for agent in all_agents:
            ...
            # Robbers use simplified direct chase to reduce CPU load.
            if isinstance(agent, RobberAI):
                obs = []
            elif self.use_dynamic_obstacles:
                for other in all_agents:
                    if other is agent:
                        continue
                    other_center = agent_centers[other.ai_id]
                    if _distance_sq(center, other_center) <= neighbor_radius_sq:
                        obs.append(agent_hitboxes[other.ai_id])
